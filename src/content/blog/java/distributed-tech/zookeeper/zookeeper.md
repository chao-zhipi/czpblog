...
**1. ZooKeeper 不适合存大量数据**  
每个 ZNode 的数据默认上限是 1MB，它是协调工具而不是数据库，不要把它当 Redis 用。

**2. Session 超时与临时节点**  
客户端 Session 超时（不是连接断开）后，临时节点才会被删除。Session 超时时间需根据业务设置，太短会导致频繁的服务注册/下线。

**3. Watch 是一次性的（原生 API）**  
原生 ZooKeeper 的 Watch 触发一次后就失效，需要在回调中重新注册。Curator 的 `CuratorCache` 解决了这个问题，会持续监听。

**4. 反射操作私有字段**  
使用 `field.setAccessible(true)` 后记得操作完设回 `false`，或使用 try-finally 保证安全。

**5. ZooKeeper 不适合高频写入**  
ZooKeeper 基于 ZAB 协议保证一致性，写操作需要集群多数节点确认，写性能不高（百级 TPS），适合低频配置变更。
